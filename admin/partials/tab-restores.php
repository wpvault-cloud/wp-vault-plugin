...
if ($wpdb->get_var("SHOW TABLES LIKE '$table'") == $table) {
    $restores = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT * FROM $table WHERE job_type = %s ORDER BY started_at DESC, created_at DESC LIMIT 50",
            'restore'
        )
    );
}
...
<div class="wpv-tab-content" id="wpv-tab-restores">
    ...
    <?php else: ?>
        <p class="wpv-backup-summary">
            <?php printf(__('Found %d restore(s) in history', 'wp-vault'), count($restores)); ?>
        </p>
...
jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.wpv-cancel-restore', function () {
        if (!confirm('<?php _e('Are you sure you want to cancel this restore?', 'wp-vault'); ?>')) {
            return;
        }
        ...
        $.post(ajaxurl, {
            ...
            if (response.success) {
                ...
                alert('<?php _e('Failed to cancel restore:', 'wp-vault'); ?> ' + (response.data.error || '<?php _e('Unknown error', 'wp-vault'); ?>'));
            }
        });
    });
