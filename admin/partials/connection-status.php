...
<div class="wpv-card wpv-connection-status">
    ...
    <div class="wpv-card-content">
        <?php if ($registered): ?>
            ...
            <?php if ($api_endpoint): ?>
                <div class="wpv-info-row">
                    <span class="wpv-info-label"><?php _e('API Endpoint:', 'wp-vault'); ?></span>
                    <span
                        class="wpv-info-value wpv-code-small"><?php echo esc_html(parse_url($api_endpoint, PHP_URL_HOST)); ?></span>
                </div>
            <?php endif; ?>
        <?php else: ?>
            <div class="wpv-status-indicator wpv-status-disconnected">
                <span class="wpv-status-dot"></span>
                <span><?php _e('Not Connected', 'wp-vault'); ?></span>
            </div>
            <p class="wpv-info-text">
                <?php _e('Please register your site in Settings to connect to WP Vault SaaS.', 'wp-vault'); ?></p>
            <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=settings'); ?>"
                class="button button-small"><?php _e('Go to Settings', 'wp-vault'); ?></a>
        <?php endif; ?>
    </div>
</div>
...
<?php if ($registered && $storage_usage): ?>
    <div class="wpv-card wpv-storage-usage">
        <div class="wpv-card-header">
            <h3><?php _e('Storage Usage', 'wp-vault'); ?></h3>
        </div>
        <div class="wpv-card-content">
            <?php
            ...
            <div class="wpv-storage-stats">
                <div class="wpv-storage-stat">
                    ...
                    <div class="wpv-stat-label"><?php _e('Used', 'wp-vault'); ?></div>
                </div>
                <div class="wpv-storage-stat">
                    <div class="wpv-stat-value"><?php echo esc_html(size_format($free_bytes, 2)); ?></div>
                    <div class="wpv-stat-label"><?php _e('Available', 'wp-vault'); ?></div>
                </div>
                <div class="wpv-storage-stat">
                    <div class="wpv-stat-value"><?php echo esc_html(size_format($quota_bytes, 2)); ?></div>
                    <div class="wpv-stat-label"><?php _e('Total', 'wp-vault'); ?></div>
                </div>
            </div>
            ...
            <div class="wpv-usage-percent">
                <?php echo esc_html($usage_percent); ?>% <?php _e('used', 'wp-vault'); ?>
            </div>
        </div>
    </div>
<?php endif; ?>