...
<div class="wrap wpv-dashboard-wrapper">
    ...
    <!-- Tab Navigation -->
    <div class="wpv-tabs">
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=dashboard'); ?>"
            class="wpv-tab <?php echo $current_tab === 'dashboard' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Dashboard', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=backups'); ?>"
            class="wpv-tab <?php echo $current_tab === 'backups' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Backups', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=restores'); ?>"
            class="wpv-tab <?php echo $current_tab === 'restores' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Restores', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=schedule'); ?>"
            class="wpv-tab <?php echo $current_tab === 'schedule' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Schedule', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=logs'); ?>"
            class="wpv-tab <?php echo $current_tab === 'logs' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Logs', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=features'); ?>"
            class="wpv-tab <?php echo $current_tab === 'features' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Features', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=settings'); ?>"
            class="wpv-tab <?php echo $current_tab === 'settings' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Settings', 'wp-vault'); ?>
        </a>
        <a href="<?php echo admin_url('admin.php?page=wp-vault&tab=storage'); ?>"
            class="wpv-tab <?php echo $current_tab === 'storage' ? 'wpv-tab-active' : ''; ?>">
            <?php _e('Storage', 'wp-vault'); ?>
        </a>
    </div>
...
<div id="wpv-progress-modal"
    ...
    <div
        ...
        <div id="wpv-log-feed"
            ...
            <div style="color:#888;"><?php _e('Waiting for logs...', 'wp-vault'); ?></div>
        </div>
    ...
    <div
        ...
        <div style="margin:20px 0; padding-top:20px; border-top:1px solid #e2e4e7;">
            ...
            <div style="margin-left:10px;">
                <label style="display:block; margin:8px 0;">
                    ...
                    <span style="color:#666; font-size:12px;"> - <?php _e('Recommended', 'wp-vault'); ?></span>
                </label>
            </div>
        </div>
        <div style="margin-top:25px; text-align:right; border-top:1px solid #e2e4e7; padding-top:15px;">
            ...
            <button type="button" class="button button-primary"
                id="wpv-confirm-restore-options"><?php _e('Start Restore', 'wp-vault'); ?></button>
        </div>
    </div>
</div>
...
<div id="wpv-restore-modal"
    ...
    <div
        ...
        <div id="wpv-restore-log-feed"
            style="background:#f8f9fa; border:1px solid #e2e4e7; border-radius:4px; height:180px; overflow:auto; padding:10px; font-family:monospace; font-size:12px; margin-top:12px;">
            <div style="color:#888;"><?php _e('Waiting for restore logs...', 'wp-vault'); ?></div>
        </div>
        ...
        $('.wpv-view-job-logs').on('click', function () {
            ...
            $.post(ajaxurl, {
                ...
                if (response.success && response.data.logs) {
                    ...
                    $('#wpv-logs-content').html('<div style="color:#d63638;"><?php _e('No logs found for this job.', 'wp-vault'); ?></div>');
                }
            }).fail(function () {
        ...
        $('#cleanup-temp-files').on('click', function () {
            ...
            $.post(ajaxurl, {
                ...
                if (response.success) {
                    ...
                    $result.html('<span style="color:red">✗ ' + (response.data.error || '<?php _e('Cleanup failed', 'wp-vault'); ?>') + '</span>');
                }
            }).always(function () {
